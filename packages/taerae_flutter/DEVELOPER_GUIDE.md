# taerae_flutter Developer Guide

This guide explains how to use `taerae_flutter` in production Flutter apps.
...
`taerae_flutter` is the Flutter-facing package for the Taerae graph stack.
...
If `taerae_flutter` is already path-based and its own `pubspec_overrides.yaml` resolves `taerae_core`, you can keep only the `taerae_flutter` path dependency in your app.
